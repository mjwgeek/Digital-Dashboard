#!/usr/bin/env python3
# Python 3.5 compatible websocket server (NO SSL version)
...
def log_flush(msg):
    if not DEBUG:
        return
    try:
        print("[websocket_server_nossl] {}".format(msg))
        try:
            import sys
            sys.stdout.flush()
        except Exception:
...
# -----------------------------
# Combined JSON builders (UNCHANGED)
# -----------------------------
...
def main():
    ...
    start_server = websockets.serve(websocket_handler, WS_BIND, WS_PORT)
    loop = asyncio.get_event_loop()
    loop.run_until_complete(start_server)
    log_flush("Websocket server is running (ws) on port {}".format(WS_PORT))
    loop.run_forever()
